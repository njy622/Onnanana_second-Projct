...
function updateLabelsForMonthController(uid) {
	...
	// 월별 라벨 업데이트 함수
	function updateLabelsForMonth(month) {
		  let labelList = [];
		  ...
		  myLineChart.data.labels = labelList;
		
		
		  // x축 눈금의 최대 표시 개수 설정 (일 수에 따라 동적으로 설정)
		  let maxTicks = daysInMonth; // 해당 월의 일 수까지 표시
		  myLineChart.options.scales.xAxes[0].ticks.maxTicksLimit = maxTicks;
		  myLineChart.options.scales.xAxes[0].ticks.max = daysInMonth + 1; // x축 최대값 설정 (해당 월의 마지막 날짜 + 1)
		  ...
		  $.ajax({
		        ...
		        success: function(result) {
		            ...
		            console.log(jsonData.userdaydata);
					myLineChart.data.datasets[0].data = jsonData.userdaydata;
		            
		            var secondDataSet = jsonData.userAlldaydata;
		            console.log(secondDataSet);
		            
		             // Modify the existing dataset instead of pushing a new one
			        if (myLineChart.data.datasets.length > 1) {
			            myLineChart.data.datasets[1].data = jsonData.userAlldaydata;
			        } else {
